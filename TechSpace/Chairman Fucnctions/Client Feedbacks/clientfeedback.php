...
if (!$result) {
    die("Error fetching client reviews: " . mysqli_error($conn));
}
...
<body>
    ...
    <div class="feed" style="display: flex; justify-content: center; gap: 60px; flex-wrap: wrap;  margin-top: 100px;">

    <?php while ($row = mysqli_fetch_assoc($result)) : ?>
    <div class="review-card" style="">
        <div class="review-header">
            <!-- Profile Placeholder (Displays First Letter of Name) -->
            <div class="profile-pic"><?= strtoupper(substr(htmlspecialchars($row['CReviewName']), 0, 1)); ?></div>
            <div class="client-info">
                <span><?= htmlspecialchars($row['CReviewName']); ?></span>
                <small><?= htmlspecialchars($row['CReviewEmail']); ?></small>
            </div>
        </div>

        <div class="review-detail">
            <p class="review-text"><?= htmlspecialchars($row['CReviewDesc']); ?></p>
        </div>
    </div>
    <?php endwhile; ?>
