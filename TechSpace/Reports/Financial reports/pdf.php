...
class MYPDF extends TCPDF {
    ...
    public function LoadBudgetData() {
        require "connect.php";
        $select = "SELECT BID, BDetails, BAmount FROM Budget";
    ...
    // Generate table for Budget
    public function GenerateBudgetTable($header, $data) {
        $this->SetFillColor(0, 51, 102);
        $this->SetTextColor(255);
        $this->SetDrawColor(0, 0, 0);
        $this->SetFont('', 'B');

        $w = array(30, 90, 40); // Adjusted for Budget Table
        ...
        $fill = 0;
        foreach ($data as $row) {
            $this->Cell($w[0], 6, $row['BID'], 'LR', 0, 'C', $fill);
            $this->Cell($w[1], 6, $row['BDetails'], 'LR', 0, 'C', $fill); // Centered this column
            $this->Cell($w[2], 6, $row['BAmount'], 'LR', 0, 'C', $fill); // Centered this column
            $this->Ln();
            $fill = !$fill;
        }
...
$pdf->SetFont('helvetica', '', 9);
$budgetHeader = array('Budget ID', 'Details', 'Amount');
$budgetData = $pdf->LoadBudgetData();
$pdf->GenerateBudgetTable($budgetHeader, $budgetData);

// Calculate Total Budget and Expense
$budgetTotal = 0;
foreach ($budgetData as $row) {
    $budgetTotal += $row['BAmount'];
}
...
$pdf->GenerateRevenueTable($revenueHeader, $revenueData);
...
$expenseTotal = $pdf->LoadExpenseData()['totalExpense'];
$expenseHeader = array('Expense ID', 'Description', 'Amount', 'Type');
$pdf->GenerateExpenseTable($expenseHeader, $expenseData);

// Calculate Remaining Budget
$remainingBudget = $budgetTotal - $expenseTotal;

// Display Money Spent, Total Expenses, and Remaining Budget
$pdf->Ln(10);
$pdf->SetFont('helvetica', 'B', 12);
$pdf->Cell(0, 10, 'Money Spent: ' . number_format($expenseTotal, 2), 0, 1, 'L');
$pdf->Cell(0, 10, 'Total Expenses: ' . number_format($expenseTotal, 2), 0, 1, 'L');
$pdf->Cell(0, 10, 'Remaining Budget: ' . number_format($remainingBudget, 2), 0, 1, 'L');
