...
class MYPDF extends TCPDF {
    ...
    public function Header() {
        ...
        $this->Cell(0, 10, 'TechSpace - Sales Report', 0, 1, 'C');
    ...
    public function LoadData() {
        ...
        $select = "SELECT 
                    ...
                    po.SalesCategoury 
                FROM sales po
                INNER JOIN products s ON po.productID = s.id;";
    ...
    public function ColoredTable($header, $data) {
        ...
        foreach ($data as $row) {
            $this->Cell($w[0], 6, $row["SalesID"], 'LR', 0, 'C', $fill);
            $this->Cell($w[1], 6, $row["SalesDate"], 'LR', 0, 'C', $fill);
            $this->Cell($w[2], 6, $row["productID"], 'LR', 0, 'C', $fill);
            $this->Cell($w[3], 6, $row["name"], 'LR', 0, 'C', $fill);
            $this->Cell($w[4], 6, $row["type"], 'LR', 0, 'C', $fill);
            $this->Cell($w[5], 6, $row["SalesCategoury"], 'LR', 0, 'C', $fill);
            $this->Cell($w[6], 6, $row["ProductUnitPrice"], 'LR', 0, 'C', $fill);
            $this->Cell($w[7], 6, $row["TotalUnitsSold"], 'LR', 0, 'C', $fill);
            $this->Cell($w[8], 6, $row["TotalAmount"], 'LR', 0, 'C', $fill);
            $this->Ln();
            $fill = !$fill;
...
$pageWidth = isset($_GET['width']) ? (int)$_GET['width'] : 290; // Default to 270mm
...
$header = array('Sales ID', 'Sales Date', 'Product ID', 'Product Name', 'Product Type', 'Category', 'Unit Price', 'Units Sold', 'Total Amount');
